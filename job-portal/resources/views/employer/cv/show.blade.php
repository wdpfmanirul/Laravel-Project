...
{{-- ══════════════════════════════════════════════════════
     CV SHOW (EMPLOYER VIEW) · "Midnight Forest" Edition
     Layout  : Matches the Candidate Preview Blueprint
     Fonts   : Syne (headings) + Literata (body)
...
.btn-print:hover { 
    ...
    box-shadow: 0 5px 18px rgba(74,124,111,.45); 
}
...
<div class="action-bar no-print">
    <div class="bar-brand">
        ...
        <span>Review Profile Details & Resume</span>
    </div>
    <div class="bar-btns">
...
<div class="cv-sheet">
    ...
    <div class="cv-left">
        ...
        <div class="photo-zone">
            <div class="photo-frame">
                @if($profile->photo)
                    <img src="{{ asset('storage/'.$profile->photo) }}" alt="Candidate Photo">
                @else
            ...
            <div class="cv-left-name">{{ $user->name }}</div>
            <div class="cv-left-job">{{ $profile->current_job_title ?? 'Professional Candidate' }}</div>
        </div>

        {{-- Profile & Contacts Information --}}
        <div class="sb-section">
            <div class="sb-head">Contact Profile</div>
            <div class="contact-list">
                ...
                <div class="contact-item">
                    ...
                    <span>{{ $profile->present_address ?? '—' }}{{ $profile->district ? ', ' . $profile->district : '' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="sb-section">
            <div class="sb-head">Personal Info</div>
            <div class="contact-list">
                <div class="contact-item">
                    <span class="contact-icon">
                ...
                <div class="contact-item">
                    <span class="contact-icon">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 11c0 3.517-1.009 6.799-2.753 9.571m-3.44-2.04l.054-.09A13.916 13.916 0 009 11V9a5 5 0 00-9.754-1.553M3 13h15M9 17h12"/></svg>
                    </span>
                    <span>{{ $profile->nationality ?? '—' }} · {{ $profile->marital_status ?? '—' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="sb-section">
            <div class="sb-head">Skills Matrix</div>
            <div class="skill-list">
                @php 
                    $skills = array_filter(array_map('trim', explode(',', $profile->skills))); 
                    $skillWidths = [90, 78, 85, 70, 88, 75, 82, 68]; 
                    $i = 0; 
                @endphp
                @forelse($skills as $skill)
                    ...
                    @php $i++; @endphp
                @empty
                    <div class="dot-item" style="color: rgba(255,255,255,.4); font-size: .7rem;">No skill listed</div>
                @endforelse
            </div>
        </div>
    ...
    <div class="cv-right">
        ...
        <div class="obj-strip">
            <p class="obj-text">
                {{ $profile->bio ?? 'Professional summary has not been configured by the candidate.' }}
            </p>
        </div>

        {{-- Professional Workspace History --}}
        ...
        {{-- Academic History --}}
        <div class="rt-section">
            <div class="rt-head"><span class="rt-head-dot"></span>Education Roadmap</div>
            <div class="edu-table">
                @forelse($profile->educations as $edu)
                    <div class="edu-row">
                        ...
                        <div class="edu-content">
                            ...
                            <div class="edu-meta">GPA/Grade: {{ $edu->cgpa_grade }} &nbsp;·&nbsp; {{ $edu->board_university ?? 'N/A' }}</div>
                        </div>
                    </div>
                @empty
                    <p style="font-size:.8rem;color:var(--muted);font-style:italic;">No formal education listed.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="rt-section">
            <div class="rt-head"><span class="rt-head-dot"></span>Trainings &amp; Certifications</div>
            <div class="edu-table">
                @forelse($profile->trainings as $training)
                    <div class="edu-row">
                        <div class="edu-years">
                            <span class="edu-yr" style="color: var(--sage);">Verified</span>
                        </div>
                        <div class="edu-content">
                            <div class="edu-degree" style="font-size: .8rem; color: var(--dark-2);">{{ $training->training_title }}</div>
                            <div class="edu-inst" style="margin-top:0px;">{{ $training->institution_name ?? 'Training Institute' }}</div>
                        </div>
                    </div>
                @empty
                    <p style="font-size:.8rem;color:var(--muted);font-style:italic;">No external certifications listed.</p>
                @endforelse
            </div>
        </div>

        {{-- Corporate Footer Declaration Stamp --}}
        <div class="declaration">
            <p class="decl-text">This resume document was rendered securely via the system matching platform pipeline.</p>
            <div class="sig-row">
                <div class="sig-date">Review Date: {{ date('d-M-Y') }}</div>
                <div class="sig-box">
                    <div class="sig-line"></div>
                    <div class="sig-lbl">System Verified</div>
                </div>
            </div>
        </div>
...
<p class="no-print" style="text-align:center;font-size:.68rem;color:#999;margin-top:22px;font-family:'Syne',sans-serif;letter-spacing:.08em;text-transform:uppercase;">
    © {{ date('Y') }} &nbsp;·&nbsp; Job Portal Corporate Verification Engine
</p>
</div>{{-- /cv-container --}}
